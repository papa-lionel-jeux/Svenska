...
function _preloadVoices() {
    if (window.speechSynthesis && window.speechSynthesis.getVoices().length === 0) {
        window.speechSynthesis.onvoiceschanged = function() {
            console.log('[TTS] Voix chargées: ' + window.speechSynthesis.getVoices().length);
        };
    }
}
if (_isIOS) _preloadVoices();
...
function _tradSpeakWebSpeech(text, lang) {
        ...
        if (voices.length === 0) {
            ...
            setTimeout(function() {
                if (window.speechSynthesis.getVoices().length === 0) {
                    var fakeVoices = window.speechSynthesis.getVoices();
                }
            }, 100);
            return null;
        }
    ...
    if (voices === null) {
        setTimeout(function() { _tradSpeakWebSpeech(text, lang); }, 200);
        return;
    }

    function doSpeak(voices) {
        if (!voices || voices.length === 0) {
            console.log('[TTS] Pas de voix dispo, utilisation défaut');
